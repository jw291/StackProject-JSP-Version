...
try{
	...
	NoticeCommentDataDAO dao = NoticeCommentDataDAO.getInstance();
	List <NoticeCommentDataBean> cList = dao.OdepthselectList(conn, b_no); 

	
	for(NoticeCommentDataBean ce : cList){
	%>
	
<div id="commentView">
<form action="NoticeBoard_Comment_Update.jsp" method="post" >
...
%>
	...
	<form id="deleteform" name="deleteform" action="NoticeBoard_Comment_Update.jsp" method="post"  style="width:버튼크기;float:right;margin-left: 5px;">
...
<form action="NoticeBoard_Comment_Update.jsp" method="post">
	...
	<div class="commentContent"><%=ce.getCo_content()%></div>
		...
		<%
			List<NoticeCommentDataBean> pList = dao.TdepthselectList(conn,ce.getCo_no());
			
			for(NoticeCommentDataBean pe : pList){
			%>
	<ul class="twoDepth">
		<li>
