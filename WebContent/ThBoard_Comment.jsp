...
try{
	...
	List <ThCommentDataBean> cList = dao.OdepthselectList(conn, b_no); 

	
	for(ThCommentDataBean ce : cList){
	%>
	
<div id="commentView">
<form action="ThBoard_Comment_Update.jsp" method="post" >
...
%>
	...
	<form id="deleteform" name="deleteform" action="ThBoard_Comment_Update.jsp" method="post"  style="width:버튼크기;float:right;margin-left: 5px;">
...
<form action="ThBoard_Comment_Update.jsp" method="post">
	...
	<div class="commentContent"><%=ce.getCo_content()%></div>
		...
		<%
			List<ThCommentDataBean> pList = dao.TdepthselectList(conn,ce.getCo_no());
			
			for(ThCommentDataBean pe : pList){
			%>
	<ul class="twoDepth">
		<li>
