...
try{
	...
	AdCommentDataDAO dao = AdCommentDataDAO.getInstance();
	List <AdCommentDataBean> cList = dao.OdepthselectList(conn, b_no); 

	
	for(AdCommentDataBean ce : cList){
	%>
	
<div id="commentView">
<form action="AdBoard_Comment_Update.jsp" method="post" >
...
%>
	...
	<form id="deleteform" name="deleteform" action="AdBoard_Comment_Update.jsp" method="post"  style="width:버튼크기;float:right;margin-left: 5px;">
...
<form action="AdBoard_Comment_Update.jsp" method="post">
	...
	<div class="commentContent"><%=ce.getCo_content()%></div>
		...
		<%
			List<AdCommentDataBean> pList = dao.TdepthselectList(conn,ce.getCo_no());
			
			for(AdCommentDataBean pe : pList){
			%>
	<ul class="twoDepth">
		<li>
